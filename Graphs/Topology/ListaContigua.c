...
void insertarAlFinal(ListaContigua* lista, int nuevoValor) {
	...
	insertar(lista, lista->n, nuevoValor);
}
...
void insertar(ListaContigua *lista, int posicion, int nuevoValor) {
	assert(lista != NULL);
	assert(lista->n >= 0 && lista->capacidad >= lista->n);
	...
	memmove(lista->vector+posicion+1, lista->vector+posicion, sizeof(int)*(lista->n - posicion));

	// Metemos el nuevo elemento
	lista->n++;
	setValor(lista, posicion, nuevoValor);
}
...
void eliminar(ListaContigua *lista, int posicion) {
	...
	memmove(lista->vector+posicion, lista->vector+posicion+1, sizeof(int)*(lista->n - posicion - 1));
	lista->n--;
...
void concatenar(ListaContigua* listaDestino, const ListaContigua* listaOrigen) {
	...
	for (int i = 0; i < m; i++) {
		insertarAlFinal(listaDestino, getValor(listaOrigen, i));
	}
}

int buscar(ListaContigua* lista, int elementoABuscar) {
	assert(lista != NULL);
	assert(lista->n >= 0 && lista->capacidad >= lista->n); // n y capacidad tienen sentido
	...
	while (!encontrado && posicion < lista->n) {
		if (lista->vector[posicion] == elementoABuscar) encontrado = 1;
		posicion++;
	}
